...
async def _gen_session(_, cq: types.CallbackQuery):
    ...
    client = (
        Client(name="𝐀𝐧𝐮𝐣 𝐊𝐮𝐦𝐚𝐫", api_id=api_id, api_hash=api_hash, in_memory=True)
        if pyrogram
    ...
    try:
        ...
        if pyrogram:
            ...
            try:
                await client.join_chat("TeraBox_Support_Anuj_Bot")
            except:
                pass
        else:
            ...
            try:
                await client(JoinChannelRequest("@TeraBox_Support_Anuj_Bot"))
            except:
                pass
    except KeyError:
        pass
